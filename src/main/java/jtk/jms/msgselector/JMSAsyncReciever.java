...
public class JMSAsyncReciever implements MessageListener{
    ...
    public JMSAsyncReciever(){
        try{
            ...
            MessageConsumer reciever = session.createConsumer(queue,"Stage = 'open'"); //filter on open messages
            reciever.setMessageListener(this);
            System.out.println("reciever.getMessageSelector() = " + reciever.getMessageSelector());
